...
public static class ImportWalletHelper
{
	...
	public static async Task<KeyManager> ImportWalletAsync(WalletManager walletManager, string walletName, string filePath)
	{
		...
		// Older CC wallet file does not contain 'ColdCardFirmwareVersion' but contains the same keys as AnonTx, so we are checking the number
		...
		KeyManager km = isColdcardJson
			...
			: GetKeyManagerByAnonTxJson(walletManager, filePath, walletFullPath);
	...
	private static KeyManager GetKeyManagerByAnonTxJson(WalletManager manager, string filePath, string walletFullPath)
	{
	...
	private static KeyManager GetKeyManagerByColdcardJson(WalletManager manager, JObject jsonWallet, string walletFullPath)
	{
		...
		// https://github.com/zkSNACKs/WalletAnonTx/pull/1663#issuecomment-508073066
		// Coldcard 2.1.0 improperly implemented AnonTx skeleton fingerprint at first, so we must reverse byte order.
