...
public class SettingsSearchSource : ReactiveObject, ISearchSource
{
	...
	private IEnumerable<ISearchItem> GetSettingsItems()
	{
		...
		yield return new ContentSearchItem(content: Setting(selector: x => x.RunOnSystemStartup), name: "Run AnonTx when computer starts", category: "Settings", keywords: new List<string>() { "startup", "boot" }, icon: "nav_settings_regular", isEnabled) { IsDefault = false, Priority = 5 };
		...
		yield return ContentSearchItemNode.Create(
			...
			nestedItemConfiguration: new NestedItemConfiguration<TorMode>(
				...
				item: new ContentSearchItem(
					...
					name: "Terminate Tor when AnonTx shuts down",
					category: "Settings",
					keywords: new List<string>(),
					icon: "nav_settings_regular",
					isEnabled)
